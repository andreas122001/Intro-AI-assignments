...
# Extended again by Andreas Bentzen Winje
...
class Map_Obj():
    ...
    def get_cell_neighbors(self, cell: tuple[int, int]) -> list[list]:
        """Gets all valid neighboring cells of a cell using 4-connectedness."""
        nb = [
            [cell[0]-1, cell[1]],
            [cell[0]+1, cell[1]],
            [cell[0],   cell[1]-1],
        ...
        return self._filter_cells(nb)
    
    def get_cell_neighbors_8(self, cell: tuple[int, int]) -> list[tuple[int, int]]:
        """Gets all valid neighboring cells of a cell using 8-connectedness."""
        nb = []
        x, y = (cell[0], cell[1]) 
        for i in range(3):
            for j in range(3):
                if (i,j)!=(1,1):
                    nb.append([ x-1+i , y-1+j ])
        return self._filter_cells(nb)

    def _filter_cells(self, cells: list) -> list[tuple[int, int]]:
        """Filters a list of cells so that all cells are valid cells."""
        # Remove cells outside the map
        cells = list(filter(lambda item: item[0] < self.int_map.shape[0] and item[1] < self.int_map.shape[1], cells))
        cells = list(filter(lambda item: item[0] >= 0 and item[1] >= 0, cells))
        
        # Remove solid cells (walls)
        cells = list(filter(lambda item: self.int_map[item[0], item[1]] != -1, cells))
        return cells
    ...
    def show_map(self, themap: Union[np.ndarray, str] = None):
        image = self.get_map_image(themap)
        image.show()

    def get_map_image(self, themap: Union[np.ndarray, str] = None):
